...
<nav class="col-md-2 menu-lateral d-flex flex-column">  
    ...
    <?php foreach($viewsController->obtenerNavsControlador($_SESSION['user_rol']) as $nav): ?>
        <li>
            ...
            <?php else: ?>
                <a class="" style="text-decoration: none; color:black;"  <?php if (!isset($nav['nav_secundaria'])) : ?> href=" <?php echo APP_URL . $nav['url_principal'] ?>" <?php endif; ?>  onclick="activarSubMenu('submenu<?= $nav['id'] ?>')"> <?= $nav['nav_principal'] ?></a>
            <?php endif; ?>
    ...
    <ul class="menu-seccion mt-auto">
        ...
        <li><i class="fas fa-sign-out-alt"> 
            <a class="" style="text-decoration: none; color:black;" onclick=" logout() ">Log-out</a>
         </i></li>  
        <li>Presential SL</li>
        <li>CheckinSign</li>
    </ul>
